...
void img::EasyImage::draw_zbuf_line(ZBuffer &zbuffer, unsigned int &x0,
                                     unsigned int &y0, double z0, unsigned int &x1,  unsigned int &y1,
                                    double z1, const Color &color) {
    ...
    if (x0 == x1)
    {
        ...
        for (unsigned int i = std::min(y0, y1); i <= std::max(y0, y1); i++)
        {
            ...
            double depth = ZBuffer::calculateDepth(i - std::min(y0, y1), a, z0, z1);
            if (zbuffer.TestAndSet(x0, i, depth))
                (*this)(x0, i) = color;
        }
    }
    else if (y0 == y1)
    {
        ...
        for (unsigned int i = std::min(x0, x1); i <= std::max(x0, x1); i++)
        {
            ...
            double depth = ZBuffer::calculateDepth(i - std::min(x0, x1), a, z0, z1);
            if(zbuffer.TestAndSet(i, y0, depth))
                (*this)(i, y0) = color;
        }
    }
    else
    {
        ...
        if (-1.0 <= m && m <= 1.0)
        {
            ...
            {
                unsigned int x = x0 + i;
                auto y = (unsigned int) round(y0 + m * i);
                if(zbuffer.TestAndSet(x, y, ZBuffer::calculateDepth(i, x1 - x0, z0, z1)))
                    (*this)(x, y) = color;
            }
        }
        else if (m > 1.0)
        {
            for (unsigned int i = 0; i <= (y1 - y0); i++)
            {
                ...
                unsigned int y = y0 + i;
                if(zbuffer.TestAndSet(x, y, ZBuffer::calculateDepth(i, y1 - y0, z0, z1)))
                    (*this)(x, y) = color;
            }
        }
        else if (m < -1.0)
        {
            for (unsigned int i = 0; i <= (y0 - y1); i++)
            {
                auto x = (unsigned int) round(x0 - (i / m));
                unsigned int y = y0 - i;
                if(zbuffer.TestAndSet(x, y, ZBuffer::calculateDepth(i, y0 - y1, z0, z1)))
                    (*this)(x, y) = color;
            }
        }
    }
...
void intersection(int &y, double &L, double &R, Vector3D &p1, Vector3D &p2) {
    if ((y - p1.y) * (y - p2.y) <= 0 and p1.y != p2.y) {
        L = p2.x + (p1.x - p2.x) * (y - p2.y) / (p1.y - p2.y);
        R = L;
    }
}

void img::EasyImage::draw_zbuf_triag(ZBuffer &zbuffer, Vector3D const& A, Vector3D const& B,
    ...
    Vector3D A2 = Vector3D::point(d * A.x / (-A.z) + dx, d * A.y / (-A.z) + dy, 0);
    Vector3D B2 = Vector3D::point(d * B.x / (-B.z) + dx, d * B.y / (-B.z) + dy, 0);
    Vector3D C2 = Vector3D::point(d * C.x / (-C.z) + dx, d * C.y / (-C.z) + dy, 0);

    int yP = roundToInt(std::max(A2.y, std::max(B2.y, C2.y)) - 0.5);
    int yQ = roundToInt(std::min(A2.y, std::min(B2.y, C2.y)) + 0.5);
    ...
    for (int y = yQ; y <= yP; ++y) {
        double xLAB = std::numeric_limits<double>::infinity() , xLAC = xLAB, xLBC = xLAB;
        double xRAB = -std::numeric_limits<double>::infinity() , xRAC = xRAB, xRBC = xRAB;

        intersection(y, xLAB, xRAB, A2, B2);
        intersection(y, xLAC, xRAC, A2, C2);
        intersection(y, xLBC, xRBC, B2, C2);

        int xL = roundToInt(std::min(xLAB, std::min(xLAC, xLBC)) + 0.5);
        int xR = roundToInt(std::max(xRAB, std::max(xRAC, xRBC)) - 0.5);

        for (int x = xL; x <= xR; ++x) {
            double z  = 1.0001 * zg + ((double)x - xg) * dzdx + ((double)y - yg) * dzdy;
